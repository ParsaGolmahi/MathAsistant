...
function checkAuth() {
    ...
    if (authToken) {
        // ✅ تنظیم توکن برای بخش اواتار
        setAvatarAuthToken(authToken);
...
async function handleRegister(e) {
    ...
    try {
        ...
        if (res.ok) {
            ...
            // اگر معلم است به صفحه معلم برود
            if (role === 'teacher') {
    ...
    showLoginPage();
        ...
        function initTabs() {
            const tabs = [
                ...
                { id: 'algebraic_identities', name: t('identities') },
...
function evaluateExpression(expr) {
    ...
    if (!input) return; // جلوگیری از خطا اگر المنت پیدا نشد

    let val = input.value.trim();
    
    // تبدیل ارقام فارسی به انگلیسی (بدون تغییر در نمایش)
    val = val.replace(/[۰-۹]/g, d => '۰۱۲۳۴۵۶۷۸۹'.indexOf(d));

    // اعتبارسنجی (فقط چک کردن عدد صحیح)
    const num = Number(val);
    if (val === "" || isNaN(num) || !Number.isInteger(parseFloat(val))) {
        alert("لطفاً یک عدد صحیح معتبر وارد کنید.");
        return;
    }

    if (num < 2) {
        alert("لطفاً عددی بزرگتر از 1 وارد کنید.");
        return;
    }
    
    // اجرای توابع اصلی (که احتمالا قبلا در فایل شما بوده)
    const isPrime = isPrimeNumber(num); 
    const divisors = findDivisors(num);
    
    // به‌روزرسانی UI (بدون تغییر در استایل‌های CSS شما)
    ...
    if (resultDiv) {
        resultDiv.innerHTML = `
            <div class="prime-result">
                <h4>${isPrime ? `عدد ${num} اول است` : `عدد ${num} مرکب است`}</h4>
                <p>شمارنده‌ها: ${divisors.join(', ')}</p>
                <p>تجزیه: ${primeFactorization(num)}</p>
            </div>
        `;
    }
}
    ...
    // 👇 اضافه کردن بخش بررسی عدد اعشاری 👇
    let val = input.value.trim();
    if (val.includes('.') || val.includes('/') || parseFloat(val) !== parseInt(val)) {
        alert(currentLanguage === 'en' ? 'Error: Please enter an integer number only.' : 'خطا: لطفا فقط عدد صحیح وارد کنید.');
        return; // توقف محاسبات
    }
    // 👆 پایان اعتبارسنجی 👆
...
function calculateGCDLCM(tabIndex) {
    ...
    // 👇 این بلاک را برای اعتبارسنجی اعداد اعشاری اضافه کنید 👇
    for (let i = 0; i < parts.length; i++) {
        let val = parts[i].trim();
        
        // بررسی وجود نقطه، ممیز (اسلش) و تفاوت مقدار اعشاری با صحیح
        if (val.includes('.') || val.includes('/') || parseFloat(val) !== parseInt(val)) {
            alert(currentLanguage === 'en' ? 'Error: Please enter integer numbers only.' : 'خطا: لطفا فقط عدد صحیح وارد کنید.');
            return; // متوقف کردن محاسبات
        }
    }
    // 👆 پایان اعتبارسنجی 👆
...
async function askTeacher() {
    ...
    try {
        ...
        if (aiAttachedFile && aiAttachedFile.mimeType.startsWith("image/")) {
            ...
            res = await fetch(`${MY_BACKEND_URL}/api/chat/vision`, {
                method: "POST",
            ...
            res = await fetch(`${MY_BACKEND_URL}/api/chat`, {
                method: "POST",
                headers: headers,
                body: JSON.stringify({
...
function clearAlgebra() {
  ...
  : 'راهنمای استفاده:\n\n1. تب مورد نظر را انتخاب کنید\n2. اطلاعات خواسته شده را وارد کنید\n3. دکمه محاسبه را بزنید\n4. نتیجه و مراحل حل را مشاهده کنید\n\nبرای اطلاعات بیشتر، به تب "درباره" مراجعه کنید.');
        ...
        function updatePageTexts() {
            ...
            // Tabs - به‌روزرسانی نام تب‌ها (ترتیب باید دقیقاً با آرایه tabs در initTabs مطابقت داشته باشد)
            const tabNames = [
                'calculator',   // 0
                null,           // 1: metaverse (دنیای ریاضی - بدون کلید ترجمه)
                null,           // 2: real (واقعیت مجازی - بدون کلید ترجمه)
                'factorial',    // 3
                'ai',           // 4
                'avatar',       // 5
                'prime',        // 6
                ...
                'vectorsTab',   // 23
                'history',      // 24
                null,           // 25: mri (MRI ریاضی - بدون کلید ترجمه)
                'geometry',     // 26
                null,           // 27: mean (میانگین - بدون کلید ترجمه)
                null,           // 28: pi (عدد پی - بدون کلید ترجمه)
                'identities',   // 29
                null,           // 30: sqrt (محاسبه جذر - بدون کلید ترجمه)
                'about',        // 31
                'settings'      // 32
            ];
...
(function() {
    ...
    // گرفتن المان اینپوت تب فاکتوریل
    const inputEl = document.getElementById('factorialInput'); 
    if (!inputEl) return;

    const numValue = inputEl.value.trim();
    ...
    // تبدیل ورودی به عدد واقعی بدون رند کردن خودکار
    const num = Number(numValue);

    // 🌟 بررسی اعشاری بودن، منفی بودن یا نامعتبر بودن عدد طبیعی
    if (isNaN(num) || num < 0 || !Number.isInteger(num)) {
        alert('لطفا عدد طبیعی وارد کنید');
        return;
    }

    // منطق اصلی محاسبه فاکتوریل شما در ادامه...
    let result = 1;
    for (let i = 2; i <= num; i++) {
        result *= i;
    }
    
    // کدهای مربوط به نمایش نتیجه در سایت شما (برای مثال):
